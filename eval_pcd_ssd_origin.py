...
def test_net(save_folder, net, pcd_process, feat_extr, cuda, dataset, transform, top_k,
    ...
    for i in range(num_images):
        ...
        gt_boxes[:, 3] *= h
        # print(gt)
        car_boxes = all_boxes[2][i][:5, :4]
        draw_result(ori_img, car_boxes, 'green')
        draw_result(ori_img, gt_boxes.astype(np.int), 'blue')
...
def draw_result(img, result, color):
    for box in result:
        if color == 'green':
            color_tuple = (0, 255, 0)
        if color == 'blue':
            color_tuple = (255, 0, 0)
        cv2.rectangle(img, (box[0], box[1]), (box[2], box[3]), color_tuple, 2)
...
if __name__ == '__main__':
    ...
    # load data
    dataset = KittiPcdDataset(args.kitti_root+'/KITTI', ['train'], BaseTransform(1280, 384, dataset_mean), AnnotationTransform())
